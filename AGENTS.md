...
| `PLANNOTATOR_CHROME_APP` | Set to `0` / `false` to stop opening sessions in a dedicated Chrome app window. Default: enabled. When on (and the session is local), `openBrowser` launches the first Chrome-family browser it finds with `--app=<url>`, which gives a frameless application window instead of a tab in the user's existing window. This has the **highest precedence** in the browser chain — ahead of the VS Code IPC fallback, Glimpse, and the system default — with two guards: it never runs for remote sessions (nothing on the user's machine to pop a window on), and it is skipped when `PLANNOTATOR_BROWSER` names a non-Chromium browser, since choosing Firefox or Safari explicitly is a deliberate opt-out. A `PLANNOTATOR_BROWSER` that *is* Chrome-family (`"Google Chrome"`, `brave-browser`, `msedge`, …) is driven in app mode instead of being replaced. When no Chrome-family browser is installed the launch returns false and the normal chain runs, so a session never fails to open because of this. The random local port is not an obstacle: Chrome keys remembered window geometry (`browser.app_window_placement`) on host **and path** — `"localhost_/"` — never on port, so a session reopens where the user left it despite `Bun.serve({ port: 0 })`, and every persisted setting is already a port-agnostic cookie rather than `localStorage` (`packages/ui/utils/storage.ts`). macOS must exec the bundle binary directly (`/Applications/Google Chrome.app/Contents/MacOS/Google Chrome`), NOT `open -a "Google Chrome" --args --app=…`, because macOS silently drops `--args` when the app is already running and would hand back an ordinary tab. Discovery lives in `packages/shared/chrome-app.ts` (vendored to Pi). Can also be set via `~/.plannotator/config.json` (`{ "chromeApp": false }`); the env var takes precedence. |
...
- `agentVariants` (array, default none) — user-declared alternate configurations of a review CLI: the same agent spawned with a different environment, so a second logged-in account appears in the **code-review** engine picker as its own provider (`{ "agentVariants": [{ "id": "codex-work", "base": "codex", "label": "Codex Work", "env": { "CODEX_HOME": "~/.codex-work" }, "accent": "violet" }] }`). Motivation: the CLIs express a second account as an env override that users wrap in a shell alias, and jobs are spawned with no shell, so an alias can never be a review engine. Shape, validation and caps (12 variants, 16 env entries) live in `packages/core/agent-variants.ts` (browser-safe); the node-side loader that reads config.json and expands `~` is `packages/shared/agent-variants-node.ts` (`-node` because Pi vendors both halves into one flat directory, where a shared module sharing a core module's basename would import itself). `base` must be one of the six review engines; at least one of `env`/`binary` is required (neither ⇒ an identical copy of the base, dropped); `id` may not collide with a built-in provider (`claude`, `codex`, `cursor`, `opencode`, `pi`, `copilot`, `tour`, `guide`, `shell`); `env` may not override `PLANNOTATOR_API_URL`/`PLANNOTATOR_AGENT_SOURCE`, which the spawn re-applies after the overlay. Invalid entries are dropped with a stderr warning and the rest still load. Each variant becomes an extra `AgentCapability` carrying `base` + `accent`, which is how the UI knows to borrow the base's icon, models and config controls (`engineBase`/`engineLabel`/`engineAccent` in `AgentsTab.tsx`) while marking it with an accent dot, ring and caption. `buildCommand` resolves a variant to its base before dispatch and `onJobComplete` parses on the base, so no downstream branch learns the word "variant"; findings are attributed to the variant's label. Read once at server start, in both runtimes (`packages/server/review.ts`, `apps/pi-extension/server/serverReview.ts`). Review-only: guide and tour keep the built-in engine set. There is no env-var equivalent and no UI to create one. See `docs/agent-variants.md`.
...
## Required Review Axes (fork)

Every built-in review, on every engine, must cover six axes — correctness, adversarial, performance, security, scope/simplification, test coverage. The list lives once in `packages/shared/review-axes.ts` (vendored to Pi) and each engine renders it in the shape its own prompt uses, because a plain append would have been dead text: Claude's hard constraints forbade flagging missing test coverage, the marker methodology filed "consider adding tests" and speculative simplification under *Do NOT report*, and none of the three prompts mentioned performance. Those suppressions were amended at their source. Every engine now gets `renderAxisChecklist()`: Codex and the marker engines have it appended in their composer — Codex's system block stays byte-identical to upstream `codex-rs/core/review_prompt.md` and is never edited in place — while Claude's prompt is ours, so the checklist sits inside it. The checklist carries an explicit "override any narrower framing" line, without which Codex's bug-focused upstream framing suppresses the performance and test-coverage axes. **Claude's review is a single pass**, like the others: it previously rendered the axes as a parallel subagent roster and then validated each candidate with its own subagent, which paid for the diff and this repo's 136KB of guidance once per agent across two sequential waves — several times the tokens and wall clock of Codex or Cursor for comparable coverage. Guideline compliance (formerly its own agent) is now a section of that one pass, and `buildClaudeCommand` withholds the `Agent` tool (absent from `--tools`/`--allowedTools`, named in `--disallowedTools`) so the fan-out cannot return by model initiative. Custom review skills are deliberately untouched (a skill replaces the methodology wholesale, the same reason `REPORTING_INSTRUCTIONS` covers only how to report). `packages/server/review-axes.test.ts` pins that every axis reaches all three engine families and that the lifted suppressions stay lifted. See `docs/review-axes.md`.
